...
cd $myperl/a2ps
cp -Rvp a2ps.pl translate $bin

cd $myperl/myconf
cp -Rvp myconf.pl $bin

cd $myperl/mycopy
cp -Rvp mycopy.pl translate $bin
...
cp -Rvp download-sortierer.py translate $bin
...
cp -Rvp active-print.py translate $bin
for i in $printers; do
